...
In the shorthand form, the key is the action plugin type (or alias/qualified name) and the value is mapped to the plugin's **default option**. Any option in the plugin manifest can be marked as `default: true`. If no option is explicitly marked, the first option defined in the manifest is assumed and a warning is logged.
