...
| `ListTriggers` | List registered triggers and their current state | filters: status |
| `ListActions` | List registered actions and their current state | filters: status |
| `ListGroups` | List registered groups | — |
| `ListRecurfiles` | List registered recurfiles | — |
...
| `InspectRecurfile` | Show recurfile details and contributed entities | by path, name, or UUID |
| `InspectEntity` | Unified inspect — resolves an identifier to whichever entity type matches and returns the per-type response in a oneof. Used by `recur inspect <id>` (no type hint). | identifier (UUID, prefix, name, or path) |
| `GetStatus` | Report daemon health, active trigger/action counts, launch args, and recent errors | — |
| `SuspendTrigger` / `ResumeTrigger` | Pause / resume a trigger | by name or UUID |
| `SuspendAction` / `ResumeAction` | Pause / resume an action | by name or UUID |
| `SuspendEntity` / `ResumeEntity` | Unified suspend/resume — same dispatch logic as `InspectEntity`. Used by `recur suspend <id>` / `recur resume <id>`. | identifier |
| `TestTrigger` / `TestAction` / `TestEntity` | Manually fire a trigger / execute an action for debugging | by name or UUID, or via the unified entity path |
| `GetConfig` | Read one or all daemon configuration values, with per-layer attribution (default / file / cli args) | optional key |
| `SetConfig` | Set a daemon configuration value (e.g., `shutdown_timeout`, `error_threshold`). The change is persisted to `config.yaml` and takes effect immediately. | key + value |
| `DeleteConfig` | Remove a daemon config key, reverting it to the default | key |
