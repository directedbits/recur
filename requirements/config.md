...
|---------|----------|-------------|
| `pkg/config/` | Generic overlay function, Store type. Separate Go module (`go.mod`). | None (stdlib only) |
...
```go
// Store holds ordered named layers and computes overlays on read.
...
```go
// MapStore is the map[string]any variant for dynamic/plugin configs.
...
| `resolveStringOption(key, triggerOpts, groupOpts, default)` | `MapStore.Get()[key]` with layers "daemon", "group", "trigger" |
| `resolveIntOption(...)` | Same |
| `resolveDurationOption(...)` | Same |
| `triggerDefaults` struct | Daemon creates `MapStore` with daemon config as the "daemon" layer |
| `config.GetEffective()` threshold fallback | `Store[DaemonConfig].Get()` with pointer fields for thresholds |
...
| Current | Replacement |
|---------|-------------|
| `config.ExplicitKeys` + `sourceAnnotation` | `Store.Inspect(field)` — `LayerValue` entries show which layer defined the value |
| `config.Get(cfg, key)` | `Store.GetField(field)` or `Store.GetLayerField(layer, field)` |
| `config.Set(cfg, key, value)` | `Store.SetField(layer, field, value)` |
| `config.Delete(cfg, key)` | `Store.ClearLayerField(layer, field)` |
...
- Store: Set/Get round-trip, layer ordering, missing layers
- Store: SetField type safety, unknown fields
- Store: thread safety (concurrent reads/writes)
- Store: Clear resets to unset
...
- `repository.go` — Store[T] type
- `map_repository.go` — MapStore type
- Full test coverage
