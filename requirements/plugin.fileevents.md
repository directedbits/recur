...
Every trigger above accepts an additional `exclude_paths: []` option
(omitted from the YAML excerpt for brevity). It takes a list of
doublestar-glob patterns and skips any matching event path. Leading
`~/` expands to the user's home; paths are matched as absolute,
slash-normalized strings.

Defaults: `["~/.config/recur/**"]` plus an unconditional filter on
any file whose basename is a Recurfile (case-insensitive, with
optional `.yaml`/`.yml` extension). Setting `exclude_paths` in
daemon config (`plugins.core.fileevents.exclude_paths`) replaces the
defaults entirely. Setting it only in a recurfile (without a
daemon-config value) is additive. Pass `[]` at either level to drop
the defaults without adding anything.
