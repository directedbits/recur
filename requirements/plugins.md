...
| `defaults` | map\<string, any\> | no | Per-trigger overrides for engine settings (`debounce`, `concurrency_mode`, `error_threshold`, `max_queue_size`, `action_error_threshold`). Sit at the bottom of the precedence chain — recurfile/group/per-trigger options override these. Used by triggers whose semantics need a non-default engine setting (e.g., `devicemonitor` sets `debounce: "0"` because hotplug events should fire immediately). |
...
| `sensitive` | bool | no | Marks the option's value as a secret. The resolved value still reaches the plugin via the stdin JSON payload (it needs it to do its job) but is **omitted from the `RECUR_*` environment** the plugin is launched with, since env vars leak via `/proc/<pid>/environ`, `ps eww`, and log lines that dump process env. Honored on both the trigger path (`src/app/recurd/triggerengine/external.go`'s `buildEnvVars`) and the action path (`src/app/recurd/daemon/action.go`'s `buildActionEnv`). Action plugins additionally treat any option whose resolved value came from a `{{secret "name"}}` template as sensitive for env purposes, even when the manifest doesn't mark it — triggers aren't template-resolved at activation time, so manifest sensitivity is the only signal there. Used for credentials like MQTT `password` and webhook `secret`. Plugins must not log option values regardless. |
