...
A multi-stage Docker image is available for containerized deployment. The build stage compiles the daemon, CLI, and a configurable set of plugins (`RECUR_PLUGINS` build arg, defaults to `timer webhook`). The runtime stage is Alpine-based, runs as a non-root `recur` user, and uses `tini` as the init process. State is persisted via a volume at `/home/recur/.recur/state`. `RECUR_SOCKET` can be set to override the default Unix socket — useful for exposing the daemon over TCP behind a proxy; the config file path is set via the daemon's `--config` flag.
