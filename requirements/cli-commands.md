...
**Recurfile commands:**
- `recur add [group] [trigger-type]` — generate (or interactively edit) a recurfile group; auto-registers with the daemon if running
...
Send a graceful shutdown signal to the running daemon. Reads the
daemon's PID from `~/.config/recur/run/recurd.pid` and sends SIGTERM.

**Behavior:**
- Errors if no daemon is running.
- After sending SIGTERM, polls the PID until the process exits or the
  configured `shutdown_timeout` elapses (default 30s; configurable in
  the daemon config). The signal handler runs the standard drain →
  persist → exit sequence regardless of how shutdown was initiated.
...
### `recur add [group] [trigger-type]`

Generate a new recurfile group and, if the daemon is running, register
it immediately. Convenience command for spinning up a recurfile from a
template without writing YAML by hand.

| Flag | Description |
|---|---|
| `--local` | Target `recur.yaml` in the current directory (default). |
| `--user` | Target `~/.config/recur/Recurfile.yaml` instead. |
| `--triggers` | Comma-separated trigger types (additional to the positional `trigger-type` arg). |
| `--actions` | Comma-separated action names. |
| `--edit` | Open `$EDITOR` after generating the stub. |
| `--stub` | Pre-populate trigger/action options from the relevant plugin manifests instead of leaving placeholders. |

**Behavior:**
- Generates a YAML fragment for the named group with the requested
  triggers and actions, validating each type against installed plugin
  manifests.
- If no triggers and no actions are specified, `$EDITOR` is opened
  unconditionally so the user can compose the fragment by hand.
- Merges the fragment with the target file (creating it if absent)
  and writes atomically via `atomicfile`.
- If the daemon is running, calls `RegisterRecurfile` so the new
  entities are live without a separate `recur register` step.
- Unknown trigger/action types fail registration with close-match
  suggestions sourced from the installed manifests.
...
See [CLI Structure — List Command Flags](cli-structure.md#list-command-flags) for `--all` details.
