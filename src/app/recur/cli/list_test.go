...
func TestExcludeTriggersByStatus(t *testing.T) {
	triggers := []*recurv1.TriggerSummary{
		{Id: "aaaaaaaa", Name: "t1", Status: recurv1.EntityStatus_ENTITY_STATUS_ACTIVE},
		{Id: "bbbbbbbb", Name: "t2", Status: recurv1.EntityStatus_ENTITY_STATUS_SUSPENDED},
		{Id: "cccccccc", Name: "t3", Status: recurv1.EntityStatus_ENTITY_STATUS_ERROR},
		{Id: "dddddddd", Name: "t4", Status: recurv1.EntityStatus_ENTITY_STATUS_SUSPENDED},
	}
	got := excludeTriggersByStatus(triggers, recurv1.EntityStatus_ENTITY_STATUS_SUSPENDED)
	if len(got) != 2 {
		t.Fatalf("expected 2 non-suspended triggers, got %d", len(got))
	}
	if got[0].Name != "t1" || got[1].Name != "t3" {
		t.Errorf("expected t1 and t3, got %s and %s", got[0].Name, got[1].Name)
	}
}

func TestExcludeActionsByStatus(t *testing.T) {
	actions := []*recurv1.ActionSummary{
		{Id: "aaaaaaaa", Name: "a1", Status: recurv1.EntityStatus_ENTITY_STATUS_ACTIVE},
		{Id: "bbbbbbbb", Name: "a2", Status: recurv1.EntityStatus_ENTITY_STATUS_SUSPENDED},
		{Id: "cccccccc", Name: "a3", Status: recurv1.EntityStatus_ENTITY_STATUS_ERROR},
	}
	got := excludeActionsByStatus(actions, recurv1.EntityStatus_ENTITY_STATUS_SUSPENDED)
	if len(got) != 2 {
		t.Fatalf("expected 2 non-suspended actions, got %d", len(got))
	}
	if got[0].Name != "a1" || got[1].Name != "a3" {
		t.Errorf("expected a1 and a3, got %s and %s", got[0].Name, got[1].Name)
	}
}
