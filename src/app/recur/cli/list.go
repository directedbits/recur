...
func newListTriggersCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if suspended, _ := cmd.Flags().GetBool("suspended"); suspended {
				triggers = filterTriggersByStatus(triggers, recurv1.EntityStatus_ENTITY_STATUS_SUSPENDED)
			}
	...
	cmd.Flags().Bool("suspended", false, "Show only suspended triggers")
	return cmd
}
...
func newListActionsCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if suspended, _ := cmd.Flags().GetBool("suspended"); suspended {
				actions = filterActionsByStatus(actions, recurv1.EntityStatus_ENTITY_STATUS_SUSPENDED)
			}
	...
	cmd.Flags().Bool("suspended", false, "Show only suspended actions")
	return cmd
}
