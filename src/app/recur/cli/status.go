...
// 1. --socket CLI flag
// 2. RECUR_SOCKET environment variable
// 3. socket_address in config file
// 4. Platform default (Unix socket on Linux, TCP on Windows)
func resolveSocketPath(cmd *cobra.Command) (string, error) {
	...
	if envPath := os.Getenv("RECUR_SOCKET"); envPath != "" {
		return envPath, nil
	}
