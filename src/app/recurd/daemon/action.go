...
func (e *pluginExecutor) Execute(ctx context.Context, a *action.Action, execCtx *executorsubprocess.Context) (*action.ExecutionResult, []string) {
	...
	// Build env vars
	var env []string
	env = append(env, fmt.Sprintf("RECUR_ACTION_TYPE=%s", a.Type))
	env = append(env, fmt.Sprintf("RECUR_LOG_LEVEL=%s", *e.config.LogLevel))
	if input.Test {
		env = append(env, "RECUR_TEST=true")
	}
	for k, v := range options {
		if result.SensitiveKeys[k] {
			continue
		}
		envKey := fmt.Sprintf("RECUR_OPT_%s", strings.ToUpper(k))
