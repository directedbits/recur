...
func TestBuildEnvVars_OmitsSensitiveOptions(t *testing.T) {
	plugin := &pluginfs.InstalledPlugin{
		ID:  "test-plugin-id",
		Dir: "/tmp/test-plugin",
		Manifest: &manifestyaml.Manifest{
			Name:      "test-plugin",
	...
	router := NewPluginEventRouter()
	factory := ExternalPluginFactory(plugin, "/tmp/test.sock", router, 0, nil)
	driver, err := factory("trigger-1", "OnConnect", map[string]any{
	...
	if lookup["RECUR_URL"] != "https://example.test" {
		t.Errorf("non-sensitive option dropped: RECUR_URL = %q", lookup["RECUR_URL"])
	}
	if _, present := lookup["RECUR_PASSWORD"]; present {
		t.Errorf("sensitive option leaked into env: RECUR_PASSWORD = %q", lookup["RECUR_PASSWORD"])
	}

	// The plugin still needs the value via stdin; verify d.options keeps it
	// so the stdinPayload built in Start() includes it.
	if d.options["password"] != "hunter2" {
		t.Errorf("d.options[password] = %v, want hunter2", d.options["password"])
	}
}
