...
func ExternalPluginFactory(plugin *pluginfs.InstalledPlugin, socketPath string, router *PluginEventRouter, shutdownTimeout time.Duration, configLookup ConfigLookup, logLevel ...string) DriverFactory {
	...
	return func(triggerID, triggerType string, options map[string]any, recurfilePath string) (Driver, error) {
		...
		// Manifest-declared sensitive option names — excluded from the
		// RECUR_<KEY> env passed to the plugin process so credentials
		// don't leak via /proc/<pid>/environ. The value still reaches the
		// plugin via the stdin JSON payload below.
		sensitiveKeys := map[string]bool{}
		if def := plugin.FindTriggerDefinition(triggerType); def != nil {
			for _, opt := range def.Options {
				if opt.Sensitive {
					sensitiveKeys[opt.Name] = true
				}
			}
		}
...
func (d *externalDriver) buildEnvVars() []string {
	...
	// Manifest-sensitive options are skipped at the top level; nested
	// keys under a sensitive map option drop out with the parent.
	filteredOptions := make(map[string]any, len(d.options))
	for k, v := range d.options {
		if d.sensitiveKeys[k] {
			continue
		}
		filteredOptions[k] = v
	}
	vars = append(vars, flattenToEnvVars("", filteredOptions)...)
